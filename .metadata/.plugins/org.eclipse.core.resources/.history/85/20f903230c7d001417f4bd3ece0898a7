...
if (WorldS){
	...
	if(Mouse.getButton() == 1 && Mouse.getX() > 326 && Mouse.getX() < 811 && Mouse.getY() > 75 && Mouse.getY() < 175){
		Playing = true;

		
	}
	if (Playing){
		g.drawImage (Play,getWidth ()/2 - Play.getWidth()/2 + 400, Play.getHeight()/2 + 25, null);
